...
**LLMの自律的な進捗追跡**: ClineなどのLLMは、このPrompt.mdの各ルールの適用状況を自律的に追跡・管理し、`apply-progress.md`ファイルに記録してください。これにより、プロジェクトがルールをどの程度適用しているかを可視化します。

#### apply-progress.mdの作成・更新ガイドライン

LLMは初回参照時および重要な作業完了時に`apply-progress.md`を作成・更新し、以下の内容を含めてください：

- **適用状況の概要**: 全体的な適用率や主要な進捗
- **各セクションごとの適用状況**: 個々のルールやガイドラインの適用状況
...
| ルール | 状態 | 備考 |
|-------|------|------|
| 👤 人間の確認 | ✅ 適用済 | プルリクエストの際に確認を実装 |
| 📅 日付の記録 | ⚠️ 部分適用 | 一部ドキュメントのみ実装 |
| 📋 変更履歴 | ❌ 未適用 | 導入検討中 |
...
| ルール | 状態 | 備考 |
|-------|------|------|
| 👤 人間の確認 | ✅ 適用済 | GitHub PRテンプレートに組み込み済み |
| 📅 日付の記録 | ✅ 適用済 | すべての主要ドキュメントに適用 |
| 📋 変更履歴 | 🔄 進行中 | メインREADMEとAPIドキュメントに導入済み |
| 🔄 最小変更 | ⚠️ 部分適用 | レビュー時の確認事項として追加 |
| ❓ 質問の推奨 | ✅ 適用済 | チームSlackに専用チャンネル設置 |
| 🚫 除外ファイル | ✅ 適用済 | .gitignoreとエディタ設定を最適化 |

### 2. 作成・更新すべき資料
| 資料 | 状態 | 備考 |
|------|------|------|
| 🎯 プロジェクトの目的と概要 | ✅ 適用済 | /docs/guide/purpose.md 作成完了 |
| 📚 現行プロダクトのドキュメント | ⚠️ 部分適用 | API仕様書は完成、他は作成中 |
| 🗺️ 機能開発ロードマップ | 🔄 進行中 | 初版をチームレビュー中 |
| 📐 機能開発のドキュメント設計 | ❌ 未適用 | 来週対応予定 |

### 3. Clineのためのルールとガイドライン
| ガイドライン | 状態 | 備考 |
|------------|------|------|
| 📖 関連ファイルの記録 | ✅ 適用済 | すべて整備完了 |
| 💾 コミットの提案とメッセージ記録 | ✅ 適用済 | コミットテンプレート導入済み |

### 4. コード理解のための手順とコツ
| 手順/コツ | 状態 | 備考 |
|----------|------|------|
| 🔍 Repomixの活用 | ❌ 未適用 | 次スプリントで導入検討 |

### 5. モノレポ構造のドキュメント
| ドキュメント | 状態 | 備考 |
|------------|------|------|
| 📦 パッケージREADME | ⚠️ 部分適用 | フロントエンドパッケージのみ作成済み |
...
**重要**: LLMは自律的にこの進捗状況を評価・記録し、改善提案を行うことでプロジェクトの品質向上に貢献してください。
