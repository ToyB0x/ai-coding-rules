...
```mermaid
graph TD
  A["ドキュメントガイドライン"] --> C1["ルール: 命名規則"]
  A --> C2["ルール: フォーマット"]
  A --> C3["ルール: 更新頻度"]
  A --> C4["ルール: 内容要件"]
  
  D["ドキュメント検証システム (Linter)"] -.-> |"参照"| C1
  D -.-> |"参照"| C2
  D -.-> |"参照"| C3
  D -.-> |"参照"| C4
...
graph TD
  ...
  D -.-> |"ルールを使って検証 & 修正"| C1["ドキュメント1"]
  D -.-> |"ルールを使って検証 & 修正"| C2["ドキュメント2"]
