...
- プロジェクト内のドキュメントを分析し、定義された種別に従って分類する
- 新しいドキュメントを作成する際は、適切な種別を選択し、その種別の特性に合わせた構造と内容を提案する
- 既存のドキュメントが種別の定義に合致しているか確認し、必要に応じて改善提案を行う
- ドキュメント種別が不明確な場合は、内容に基づいて適切な種別を提案する
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- プロジェクトのドキュメント構造を分析し、このルールに準拠しているか確認する
- 新しいドキュメントを作成する際は、この構造に従って適切な配置場所を提案する
- ドキュメントの移動や再構成が必要な場合は、この構造に基づいた移動先を提案する
- 対象読者別のアクセスパスを考慮し、適切なナビゲーションリンクを提案する
- ドキュメント間の関連性を考慮し、相互参照を適切に設計する
- 時間軸（現在の機能と未来の機能）に基づいた分類を維持する
- ルールとコンテンツの分離原則に従い、ドキュメントの適切な配置を確保する
```
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- 開発プロジェクトの現在のステージを特定し、そのステージで必要なドキュメントを確認する
- 各ステージで作成・更新すべきドキュメントの内容と形式を提案する
- 次のステージへの移行条件を確認し、満たされているかどうかを評価する
- ドキュメント作成の責任者に適切なガイダンスを提供する
- 開発ライフサイクルの各ステップで必要なドキュメントテンプレートを提供する
- 既存のドキュメントが開発フローのどのステージに対応しているかを分析する
- 開発プロセスの改善提案を行う際は、ドキュメント作成フローも考慮する
```
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- ドキュメント作成・更新の際は、事前に既存の構造と内容を理解し、更新計画を提示する
- 既存の書式やスタイルを一貫して適用し、メタデータとChangelogを必ず更新する
- 変更箇所を明示し、人間のレビューを促進する形で提案を行う
- ドキュメント参照時は、重要ドキュメント一覧から始め、構造化された読み込みを行う
- メタデータを活用して最新性と信頼性を評価し、相互参照を追跡する
- 複数のドキュメントを参照する際も一貫した理解を維持する
- 自身の強み（大量処理、パターン認識、一貫性チェック）を活かし、人間の強み（戦略的判断、暗黙知）を補完する
- 効果的な協働のためのプラクティスに従い、下書き作成と実装を担当する
```
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- 大規模コードベースを理解する必要がある場合、MCPを通じてRepomixを活用する
- 特定のパッケージやディレクトリを分析する際は、Repomixを使用して効率的に構造を把握する
- 数十ファイル以上のパッケージを分析する場合は、関数シグネチャのみの分析を優先し、通信負荷を軽減する
- リポジトリの規模や分析の目的に応じて、適切な分析範囲（全体/パッケージ/ディレクトリ）を選択する
- MCPが未設定の場合は、ユーザーにRepomixのセットアップ方法を説明し、設定を促す
- 分析結果を基に、コードベースの構造や依存関係を明確に説明する
```
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- 作業完了時には必ずコミットを提案し、変更内容を明確に伝える
- コミットメッセージは「種類: 要約」の標準形式に従い、詳細な説明と変更ファイル一覧を含める
- プロンプト履歴をコミットメッセージに含め、変更の背景や理由を明確にする
- コミット提案時にはユーザーの承認を得てから実行する
- 複数の関連する変更がある場合は、論理的にまとまったコミット単位を提案する
- 変更の種類（feat, fix, docs, style, refactor, test, chore など）を適切に選択する
```
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- プロジェクトでClineを使用している場合、設定ファイルの存在と内容を確認する
- プロジェクトでRoo Codeを使用している場合、設定ファイルの存在と内容を確認する
- 設定ファイルが存在しない場合は、適切な設定ファイルの作成を提案する
- 設定ファイルの内容に問題がある場合は、修正案を提示する
- AIツールの最適な設定について、プロジェクトの特性に合わせたアドバイスを提供する
- 設定ファイルの変更がプロジェクトに与える影響を説明する
```
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- プロジェクトに.llm-guides/ディレクトリが存在するか確認し、存在しない場合は作成を提案する
- 重要ドキュメント一覧（important-docs.md）を最初に参照し、プロジェクトの全体像を把握する
- 提供されているプロンプトテンプレートを活用し、一貫性のある出力を生成する
- メタドキュメントディレクトリの構造に従い、必要に応じて新しいガイドやテンプレートの追加を提案する
- プロジェクト固有のルールやガイドラインを理解し、それに従って行動する
- コンテキスト制限を考慮し、効率的な情報収集と処理を行う
- メタドキュメントの内容が古くなっている場合は、更新を提案する
```
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- ドキュメントを更新する際は、依存関係メタデータを確認し、影響範囲を把握する
- 新しいドキュメントを作成する際は、「ドキュメント関係」セクションを追加し、関連ドキュメントへの参照を明示する
- 参照と被参照の双方向関係を一致させ、整合性を維持する
- 依存関係の種類（直接参照、概念参照、派生関係）を明確に区別して記載する
- ドキュメント間の依存関係を可視化し、知識ネットワークの全体像を把握する
- 孤立したドキュメントや循環参照を検出した場合は、修正を提案する
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- 新しいドキュメントを作成する際は、適切なテンプレートを選択して活用する
- テンプレートの構造と形式を尊重し、一貫性のあるドキュメントを作成する
- 参考例を分析し、ドキュメントの種類ごとの特徴と必要な内容を理解する
- ドキュメント作成時に、参考例を参照して内容の充実度や品質を向上させる
- テンプレートに不足している項目がある場合は、追加を提案する
- プロジェクトの進化に合わせてテンプレートの更新を提案する
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- 新しいドキュメントを作成する際は、標準メタデータセクションを必ず含める
- ドキュメントを更新する際は、メタデータの最終更新日を現在の日付に更新する
- ドキュメントのステータスを適切に設定し、必要に応じて変更する
- バージョン番号をセマンティックバージョニングに従って更新する
- メタデータを活用してドキュメントの最新性と信頼性を評価する
- メタデータが不足しているドキュメントを発見した場合は、追加を提案する
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- コード変更を提案する際は、関連するドキュメントの更新も同時に提案する
- コード変更によって影響を受けるドキュメントを特定し、更新範囲を明確にする
- APIやデータモデルの変更を行う場合は、対応するドキュメントの更新を優先的に行う
- 大きな変更の場合は、ドキュメント更新計画を作成し、段階的な更新を提案する
- コミットメッセージやPR説明文に、ドキュメント更新の内容も明記する
- ドキュメント更新が不十分なコード変更を発見した場合は、更新の必要性を指摘する
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- ドキュメントポータル（docs/README.md）の構造と内容を理解し、必要に応じて改善を提案する
- 新しいドキュメントを作成する際は、適切なブレッドクラムを追加して階層構造を明示する
- ドキュメントの末尾に関連ドキュメントセクションを追加し、関連性の説明を含める
- ディレクトリには必ずREADME.mdを作成し、そのディレクトリ内のコンテンツの概要と目次を提供する
- 相対パスを一貫して使用し、リポジトリの移動や再構成に強い参照を維持する
- 階層構造が深くなりすぎないよう、3〜4レベルを超えないように設計する
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- AIプロンプトベースの検証システムを活用し、ドキュメントの品質と一貫性を検証する
- 依存関係分析、参照関係更新、メタデータ検証などの機能を適切に使い分ける
- 指定された形式（@docs/rules/[ルール名]/ @[TARGET_DIR]）でプロンプトを構成する
- 検証結果を明確に報告し、問題点と改善案を提示する
- メタデータの完全性、フォーマットの一貫性、リンクの有効性などの検証項目を確認する
- 定期的なドキュメント品質レビューを提案し、全体的な品質向上に貢献する
...
## AIインストラクション
このルールを読んだAIは以下の行動をとってください：
- ドキュメントルールの適用状況を分析し、現在の進捗状況を把握する
- 進捗管理ダッシュボードの情報を活用して、優先的に対応すべき領域を特定する
- 各ルールの適用状況（適用割合、トレンド、課題など）を確認し、改善提案を行う
- 優先順位付けの基準（影響範囲、更新頻度、技術的依存、実装容易性）に基づいて提案を行う
- 実装フェーズ（基盤整備、主要ドキュメントの整備、拡張と最適化）を考慮した段階的な改善を提案する
- 定期的な進捗レポートの更新を促し、状況の可視化を支援する
