...
```mermaid
graph TD
  A[ドキュメント基本方針] --> C1[ルール: 命名規則]
  A --> C2[ルール: フォーマット]
  A --> C3[ルール: 更新頻度]
  A --> C4[ルール: 内容要件]
  
  D[ドキュメント検証システム (Linter)] -.-> |チェック & 適用| C1
  D -.-> |チェック & 適用| C2
  D -.-> |チェック & 適用| C3
  D -.-> |チェック & 適用| C4
...
- **ドキュメント基本方針**: ドキュメント管理の根本的な目的、価値観、長期的ビジョンを定義するもの。各プロジェクトの目的や規模に応じて独自に策定する
- **ドキュメントルール**: ドキュメント作成・更新・管理のための具体的で検証可能な個別ルール。ドキュメント基本方針から導出される。一般的に適用可能なルールとして以下がある：
  - ドキュメント種別に関するルール
  - ドキュメント配置構造に関するルール
  - フォーマットに関するルール
  - 命名規則に関するルール
...
graph TD
  ...
  D[ドキュメント検証システム (Linter)] -.-> |チェック & 適用| B1
  D -.-> |チェック & 適用| B2
  D -.-> |チェック & 適用| B3
  D -.-> |チェック & 適用| B4
  D -.-> |チェック & 適用| B5
...
1. **ドキュメンテーションガイドラインの策定**
   ...
   - 基本方針策定（ドキュメント構造、ディレクトリ構造、各種規約など）
   - 既存基本方針の改善検討
...
## 2025/03/23
- [追加] ドキュメント管理の概念モデルを追加（ドキュメント基本方針、ドキュメントルール、ドキュメント検証システム）
- [変更] 用語を統一（ドキュメンテーションガイドライン→ドキュメント基本方針）
